...
What it produces, per input page:
  ...
  * a merged `INDEX.md` row: `| `file.md` | summary | keywords |` — the single
    ...
    this tool only ever adds/updates DATA ROWS in the Entries table — it never
    rewrites the protocol prose, and on an EXISTING INDEX.md it preserves every
    other line byte-for-byte: header, Categories/Provenance sections, anything
    a fork has added. Its own `INDEX_HEADER` is used ONLY when creating a
    brand-new INDEX.md from scratch — see `merge_index_text()`).
...
Filename-collision safety (a NEW page's slug matching an EXISTING file's name):
  ...
  provenance line this tool writes into every entry it produces). Re-ingesting a
  page that PREVIOUSLY collided is also recognized as the same case, not a new
