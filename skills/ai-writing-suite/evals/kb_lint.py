#!/usr/bin/env python3
"""Lint the knowledge base's add-an-entry contract (P2.5 KB wiki).

WHY this exists: the product promise is "a company drops in a markdown page and
edits a table — no retrieval engine to build" (design D5/D11). That promise only
holds if the add-an-entry contract is mechanically enforceable, because a fork
edits the KB by hand with no embeddings/build step to catch mistakes. A typo in
an INDEX row, a `## Related entries` link that points at a deleted file, or a
one-way link silently degrades retrieval with NO runtime error — the agent just
reads a worse index or a dead hop. This script turns each of those silent
failures into a red check, the same way smoke_test.py guards the retrieval chain.

It is the companion to smoke_test.py: smoke_test proves retrieval *resolves*
correctly over the shipped KB; kb_lint proves the KB *structure* a fork must
maintain is internally consistent. Stdlib-only, zero deps, no embeddings — it
runs identically on every target surface (D5).

The four checks (each a function returning `(ok, problems)` so the unittest in
test_kb_wiki.py can import and assert them without shelling out):
...
The default KB path resolves relative to this script, so it works from any cwd.
"""
...
# evals/ -> <skill-root> -> _shared/knowledge. Resolve relative to THIS file so
# the default works from any cwd (same convention as smoke_test.py / run_all.sh).
...
def _index_rows(kb):
    ...
    Same row shape smoke_test.py keys on: `| `name.md` | summary | keywords |`.
...
def check_keywords(kb):
    ...
    The Keywords/aliases column is the primary retrieval signal (smoke_test.py
    scores keyword overlap first). An empty cell means that entry can only be
    reached by Summary-intent overlap — a silent recall hole a fork would not
    otherwise see."""
    problems = []
...
# Ordered registry so main() and the test iterate the same checks identically.
CHECKS = [
